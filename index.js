...
function payParser(ancestor, elementName, xpath) {
  const pay = cdataParser(ancestor, elementName, xpath, 'pay', 'payText', (match, cdataPay) => {
    ...
    if (reMatch) {
      return {
        ...
        pay: {
          ...cdataPay.pay,
          [reMatch.groups.suboccupation]: +(+((reMatch.groups.pay.replace(',', '')) / 2080).toFixed(2)),
        },
      };
    }
    ...
    if (reMatch) {
      return {
        ...
        pay: {
          ...cdataPay.pay,
          [reMatch.groups.suboccupation]: Number(reMatch.groups.pay),
        },
      };
    }
...
function similarOccupationsParser(ancestor, elementName, xpath) {
  ...
  return soElements.map((e) => e.textContent.trim());
}
